...
class ChatSidebarTest {
    ...
    // Easing formulas match ChatSidebar.animProgress() internals.
    // Opening: easeOutCubic — 1 - (1-t)^3
    // Closing: reversed easeInQuad — 1 - t^2
    ...
    void easing_close_mid() {
        assertEquals(0.75f, animCloseEasing(0.5f), 0.001f);
    }
    ...
    void easing_openMatchesAnimationProgress() {
        for (int i = 0; i <= 100; i++) {
            float t = i / 100f;
            long pretendStart = 0;
            // Animation.progress() returns easeOutCubic(t) for opening
            // ChatSidebar.animProgress() returns 1-(1-t)^3 for opening
