 ...
 * 与 vanilla blitNineSliced（tile 平铺，渐变纹理露馅）不同，stretch 对任意贴图形状不变形。
 ...
 * 渲染用 POSITION_COLOR_TEX 顶点：tint 色动态着色（白色默认纹理 × 主题色），alpha 通道动态淡入淡出。
 */
public final class NineSliceRenderer {
    ...
    private static void quad(BufferBuilder bb, Matrix4f pose,
        ...
        float u1 = (u + sW) * uw;
        float v1 = (v + sH) * vh;
        bb.vertex(pose, x, y, 0).color(r, gr, b, a).uv(u0, v0).endVertex();
        bb.vertex(pose, x, y + h, 0).color(r, gr, b, a).uv(u0, v1).endVertex();
        bb.vertex(pose, x + w, y + h, 0).color(r, gr, b, a).uv(u1, v1).endVertex();
        bb.vertex(pose, x + w, y, 0).color(r, gr, b, a).uv(u1, v0).endVertex();
    }
}
