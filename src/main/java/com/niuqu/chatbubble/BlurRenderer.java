 ...
 *
 * In Minecraft 1.21.11+, the GPU copy API (CommandEncoder.copyTextureToTexture)
 * only supports square region copies. The blur effect uses GL30.glBlitFramebuffer
 * which is still available via LWJGL, but the framebuffer layout changed.
 * Disabled for now.
 */
public class BlurRenderer {
    ...
    public static void blurPanel(int x, int y, int w, int h) {
        //#if MC >= 12111
        // Blur disabled in 1.21.11 - GPU copy API changed
        //#else
