...
public class ChatMessageStore {
     ...
     *  never touch MinecraftClient.getInstance() (null in the test JVM). */
    static java.util.function.Supplier<File> gameDirSupplier = null;
    ...
    // the expensive styled-JSON serialization and the full-file rewrite stay off
    // the render path. One thread preserves ordering (the tmp file is never
    // written concurrently), and it is daemon so it never holds the JVM open.
