...
public final class EmoteCatalog {
    ...
    private static final Map<String, Identifier> BUILTIN_TEXTURES = new HashMap<>();
    ...
    public static void ensureRegistered() {
        ...
        for (String token : BUILTIN) {
            try {
                ...
                Identifier id = Identifier.of("e33chat", "emote/" + token);
    ...
    public static Identifier resolve(String token) {
        if (token == null || token.isBlank()) return null;
        ensureRegistered();
        Identifier id = BUILTIN_TEXTURES.get(token);
    ...
    private static NativeImage drawPixelEmote(String token) throws Exception {
        ...
        for (int y = 0; y < 16; y++) {
            for (int x = 0; x < 16; x++) {
                int c = argb[y * 16 + x];
                ni.setColor(x, y, (c & 0xFF00FF00) | ((c & 0x00FF0000) >> 16) | ((c & 0x000000FF) << 16));
            }
        }
        return ni;
    }
