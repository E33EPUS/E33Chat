...
- 📝 **输入保留** — 关闭聊天框保留已输入文本
- 🧩 **服务端消息格式模板** — 服务端声明聊天格式，客户端精确解析插件/NCR 改过的消息（装饰名、私聊方向、`>>`/冒号/`»` 通吃）；服务端配置 GUI（`/e33chat gui`）+ 命令管理，详见 [服务端消息格式模板](#服务端消息格式模板)
...
> 聊天记录分发、改用 `/tpa` 和模板功能需要在服务端配置文件手动启用
...
- **模板层前置**（2.2.6+）：服务端配置了模板时，系统通道消息先按模板精确解析——命中即跳过守卫链；未配置或未命中再落守卫（见 [服务端消息格式模板](#服务端消息格式模板)）
